@extends('sj_layout')
@section('title'){{ __('OrderCharges') }}@endsection
@section('content')
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>@if ($entity->getOrder() && $entity->getOrder()->isPending())
                {{ Form::select("type[$i]", [null => __('selecciona'), \App\Entities\OrderCharge::TYPE_INVOICED => \App\Entities\OrderCharge::typeName(\App\Entities\OrderCharge::TYPE_INVOICED)], old("type[$i]", $entity->getType()), ['class'=>'form-select form-select-sm' . ($errors->has('type[$i]') ? ' is-invalid': '')]) }}
            @else
                {{ $entity->getTypeName() }}
            @endif</td>
            <td>@if ($entity->getOrder() && $entity->getOrder()->isPending())
